...
static void OnNvmDataChange(uint16_t flags) 
{
	LOG_INF("OnNvmDataChange flags=0x%04X", (int)flags);
	...
	if (status != LORAMAC_STATUS_OK) {
		LOG_ERR("Crypto status fetch failed: %d(%s)",
			(int)status, lorawan_status2str(status));
		return;
	}
	//LoRaMacNvmData_t *nvm = mibGet.Param.Contexts;
	
	// TODO nocommit: store key elements to NVM
/*
	if (flags & LORAMAC_NVM_NOTIFY_FLAG_CRYPTO) {
	LOG_INF("TODO: save crypto state");
		LoRaMacCryptoNvmData_t *crypto = &nvm->Crypto;
		LOG_INF("Crypto NVM:  LrWanVersion=0x%08X",(uint32_t)crypto->LrWanVersion.Value);
		LOG_INF("Crypto NVM:      devNonce=0x%04X",(uint32_t)crypto->DevNonce);
		LOG_INF("Crypto NVM:     joinNonce=0x%08X",(uint32_t)crypto->JoinNonce);
		LOG_INF("Crypto NVM:        FCntUp=0x%08X",(uint32_t)crypto->FCntList.FCntUp);
		LOG_INF("Crypto NVM:     NFCntDown=0x%08X",(uint32_t)crypto->FCntList.NFCntDown);
		LOG_INF("Crypto NVM:     AFCntDown=0x%08X",(uint32_t)crypto->FCntList.AFCntDown);
		for (int c=0; c<LORAMAC_MAX_MC_CTX; c++) {
			LOG_INF("Crypto NVM:  McFcntDown.%d=0x%08X",c, (uint32_t)crypto->FCntList.McFCntDown[c]);
		}
#if( USE_LRWAN_1_1_X_CRYPTO == 1 )
		LOG_INF("Crypto NVM:     RJCount1=0x%08X",(uint32_t)crypto->FCntList.RJcount1);
#endif
		LOG_INF("Crypto NVM: LastDownFCnt=0x%08X",(uint32_t)crypto->LastDownFCnt);

		LOG_INF("Crypto NVM:	Crc32=0x%08X",(uint32_t)crypto->Crc32);
	}
	if (flags & LORAMAC_NVM_NOTIFY_FLAG_MAC_GROUP1) {
	LOG_INF("TODO: save group1 state");

		LoRaMacNvmDataGroup1_t *mac1 = &nvm->MacGroup1;
		LOG_INF("Group1 NVM:     AdrAckCounter=0x%08X", (uint32_t)mac1->AdrAckCounter);
		LOG_INF("Group1 NVM:    LastTxDoneTime=0x%08X", (uint32_t)mac1->LastTxDoneTime);
		LOG_INF("Group1 NVM: AggregatedTimeOff=0x%08X", (uint32_t)mac1->AggregatedTimeOff);
		LOG_INF("Group1 NVM:         LastRxMic=0x%08X", (uint32_t)mac1->LastRxMic);
		LOG_INF("Group1 NVM:   ChannelsTxPower=0x%02X", (uint32_t)mac1->ChannelsTxPower);
		LOG_INF("Group1 NVM:  ChannelsDatarate=0x%02X", (uint32_t)mac1->ChannelsDatarate);
		LOG_INF("Group1 NVM:   SrvAckRequested=%d",     (uint32_t)mac1->SrvAckRequested);
		LOG_INF("Group1 NVM:             Crc32=0x%08X", (uint32_t)mac1->Crc32);
		//LOG_HEXDUMP_DBG(&nvm->MacGroup1, sizeof(nvm->MacGroup1),
		"nvm.MacGroup1");
	}
	if (flags & LORAMAC_NVM_NOTIFY_FLAG_MAC_GROUP2) {
		LOG_WRN("Saving not implemented for nvm.MacGroup2");
		//LOG_HEXDUMP_DBG(&nvm->MacGroup2, sizeof(nvm->MacGroup2), "nvm.MacGroup2");
	}
	if (flags & LORAMAC_NVM_NOTIFY_FLAG_SECURE_ELEMENT) {
		LOG_WRN("Saving not implemented for nvm.SecureElement");
		//LOG_HEXDUMP_DBG(&nvm->SecureElement, sizeof(nvm->SecureElement), "nvm.SecureElement");
	}
	if (flags & LORAMAC_NVM_NOTIFY_FLAG_REGION_GROUP1) {
		LOG_WRN("Saving not implemented for nvm.RegionGroup1");
		//LOG_HEXDUMP_DBG(&nvm->RegionGroup1, sizeof(nvm->RegionGroup1), "nvm.RegionGroup1");
	}
	if (flags & LORAMAC_NVM_NOTIFY_FLAG_REGION_GROUP2) {
		LOG_WRN("Saving not implemented for nvm.RegionGroup1");
		//LOG_HEXDUMP_DBG(&nvm->RegionGroup2, sizeof(nvm->RegionGroup2), "nvm.RegionGroup2");
	}
	if (flags & LORAMAC_NVM_NOTIFY_FLAG_CLASS_B) {
		LOG_WRN("Saving not implemented for nvm.ClassB");
		//LOG_HEXDUMP_DBG(&nvm->ClassB, sizeof(nvm->ClassB), "nvm.ClassB");
	}
*/	
...
int lorawan_start(void)
{
	...
	/* Restore OTAA counter and nonce state */
	LoRaMacNvmData_t *Contexts = NULL;// todo nocommit
	mib_req.Type = MIB_NVM_CTXS;
	mib_req.Param.Contexts = Contexts;
	status = LoRaMacMibSetRequestConfirm(&mib_req);
	if (status != LORAMAC_STATUS_OK) {
		LOG_ERR("Error restoring LoRaWan context: %d(%s)", (int)status, lorawan_status2str((int)status));
	}
